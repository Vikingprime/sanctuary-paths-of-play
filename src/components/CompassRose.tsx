...
interface CompassRoseProps {
  size?: number;
  className?: string;
  rotation?: number; // Rotation in degrees (for landscape mode)
}

// Simple compass rose icon showing N/S/E/W
// Supports rotation prop for when the map is rotated (landscape mode)
export const CompassRose = ({ size = 60, className = '', rotation = 0 }: CompassRoseProps) => {
  return (
    <div 
      className={`relative flex items-center justify-center ${className}`}
      style={{ width: size, height: size }}
    >
      {/* Rotating container for compass directions */}
      <div 
